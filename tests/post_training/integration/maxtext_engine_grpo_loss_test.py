...
def _create_training_engine() -> maxtext_engine.MaxTextTrainingEngine:
  """Creates a MaxTextTrainingEngine with the GRPO loss and TunixMaxTextAdapter model."""
  cfg = _config()
  mesh = jax.sharding.Mesh(maxtext_utils.create_device_mesh(cfg), cfg.mesh_axes)
  return maxtext_engine.MaxTextTrainingEngine(
      cfg,
      mesh=mesh,
...
class MaxTextEngineGrpoLossTest(absltest.TestCase):
  ...
  def test_grpo_loss_drives_a_training_step(self):
    ...
    engine = _create_training_engine()
