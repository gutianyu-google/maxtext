...
This module provides interception and wrapping hooks for monitoring tensor
distributions, activations, and routing statistics across Flax Linen and NNX
model layers in MaxText without modifying layer implementations.

Why This Is Needed:
  Debugging numerical anomalies (e.g., NaNs, Infs, vanishing/exploding
  activations, and imbalanced MoE router weights) in large-scale distributed
  training traditionally requires manually placing debug logging statements or
  probes throughout the model definition. This is error-prone, intrusive, and
  clutters architecture code.

  This module solves this by providing non-intrusive, automated instrumentation
  for both Flax paradigms used in MaxText:
  1. Flax Linen: Intercepts module execution using `nn.intercept_methods` to
     capture intermediate forward outputs and auxiliary tensors (such as KV
     caches and MoE load-balancing loss).
  2. Flax NNX: Traverses module object graphs via `nnx.iter_graph` and wraps
     module instances, hooking `__call__` and MoE routing methods (`get_topk`,
     `reshape_and_update_weights`) to capture layer outputs, router weights,
     and combine weights.

  Both interceptors integrate with `debug_telemetry_scope`, introducing zero
  overhead at trace time when disabled, and conditionally forwarding tensors
  to `debug_tensor` when active according to configured step intervals and
  layer filters.

Key Functions and Classes:
  - debug_telemetry_scope: Context manager that manages thread-local telemetry
    state (config and current step) and enables Linen method interception.
  - linen_interceptor_fn: Linen interceptor callback invoked around module calls
    to attach hierarchical tags (e.g., `decoder/layers_0/...`) and instrument
    outputs.
  - wrap_nnx_module_for_debug: Traverses an NNX module hierarchy and subclasses
    modules to transparently intercept forward calls and MoE routing methods.
  - is_debug_telemetry_active: Queries whether telemetry logging is currently
    active in the local thread.
  - get_active_telemetry_context: Returns the active configuration and step.
"""
