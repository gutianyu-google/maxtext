...
def transform_logic(path: Tuple[str, ...], shape: Optional[Tuple[int, ...]] = None) -> Optional[mdn]:
  ...
  # Exclude non-matrix parameters, embeddings, biases, normalization, and routing gates
  if any(
  ...
  # Standard 2D matrix weights (dense MLPs, MoE routed experts, GDN, shared experts)
