...
def _normalize_loss_output(out: Any, has_aux: bool) -> abstract_engine.LossOutput:
  ...
  Args:
    ...
    has_aux: Whether the caller considers a 2-tuple's second element to be auxiliary
      output worth recording. When False, an aux read here to build the primary loss is
      still dropped from `aux_metrics` -- see `with_loss_fn`.
...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _eval_kernel(self, params, rest, batch) -> tuple[abstract_engine.WeightedMetric, dict[str, Any]]:
  ...
  def _compile_eval_for_batch(self, dynamic_batch: Any, static_batch: dict[str, Any]) -> None:
    ...
    Separate from `_compile_for_batch` because eval runs on its own batch shapes and its
    own schedule: it is built on the first `eval_step`, not when `compile()` is called,
    and the caller of `compile()` has no eval payload to offer anyway.
    """
    self._model_graphdef, params_pure, rest_pure = nnx.split(self._model, nnx.Param, ...)
  ...
  def eval_context(self):
    ...
    Unlike Tunix's equivalent this sets no ambient mode. `eval_step` names the mode on every
    `record_metrics` call, so a caller that forgets this context still records into the eval
    recorder rather than silently writing eval numbers into the train buffer.
    """
  ...
  def eval_step(self, payload: abstract_engine.TrainerPayload, **kwargs: Any) -> None:
    ...
    Prepares inputs, runs the eval function, and logs eval metrics.
