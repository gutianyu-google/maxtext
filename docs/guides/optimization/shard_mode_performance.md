...
04. **The penalty does not simply amortize with scale.** Its *instruction count* is fixed (always the
    same six copies); its *cost* is not, and the ladder is **non-monotone**: w512 +4.80% → w2048
...
10. **There is a fix, it is measured, and it ships here off by default.** The barrier itself cannot be
    ...
    "stop pinning the logits output" recommendation was tested and produces a 0-line HLO diff). What
    can be removed is the *need* for the fold: store the untied LM head kernel as `[vocab, embed]`
    and autodiff produces the weight gradient already in the kernel's orientation.
    `lm_head_kernel_transposed: true` does that, and it takes dp4 from **+9.34% to −0.23%** and
    emb4096 from **+1.56% to +0.56%**, with the six relayout copies going to zero. It defaults off
    because it also changes the checkpoint's on-disk orientation, which hands `[V, embed]` to `auto`
    too — where it is a regression. See §8 item 1 for the numbers and the follow-up that lifts that
    restriction.
...
and control the checkpoint format, set `lm_head_kernel_transposed: true` (§8) rather than assuming
scale will hide it.
...
1. **The ladder is non-monotone.** +4.80% → +0.96% → **+1.49%** along width; +0.96% → +1.74% →
   +0.32% along sequence. The largest *absolute* penalty in the entire 28-run sweep (**+1111 µs**
   against a 2-run auto mean of 72,171 µs) is at the **largest** width. A fixed-cost model
   calibrated at w512 (+192 µs) predicts d16 = +0.24% and w4096 = +0.27%; measured are +0.088% and
   +1.54%, i.e. **wrong by 2.7× low and 5.7× high**.
...
- **Pure data parallelism:** prefer `auto`, or set `lm_head_kernel_transposed: true` if you are
  committed to explicit and control the checkpoint format — that closes the whole +9.3% gap (§8
  item 1). +10% is not noise.
...
01. **Store the untied LM head kernel as `[vocab, embed]` under explicit — `lm_head_kernel_transposed`
    (§4.1, §4.3, §4.5).**

    > An earlier revision of this list recommended "stop pinning the logits output under explicit."
    > **That is measured false** and has been withdrawn — see the correction at the head of §4.5.
    > Gating the `out_sharding` produces a 0-line HLO diff; the barriers are JAX's, not MaxText's.

    Since the barrier cannot be removed, remove the *need* for the fold instead. If the kernel is
    already `[vocab, embed]`, autodiff produces the weight gradient in the kernel's own orientation
    and there is no transpose for `algsimp` to fail to fold. `DenseGeneral(kernel_transposed=True)`
    implements this; `lm_head_kernel_transposed: false` in `base.yml` wires it to the untied head.

    Measured on v5p-8, 4 chips, xprof `train_step_ns` median on `/device:TPU:0`
    (`[E,V]` = today, `[V,E]` = transposed):

    | config         | explicit `[E,V]` | explicit `[V,E]` | Δ explicit | penalty before → after |         recovery |
    | -------------- | ---------------: | ---------------: | ---------: | ---------------------: | ---------------: |
    | llama2 dp4     |           4851.4 |           4426.8 | **−8.75%** |        +9.35% → −0.03% |             102% |
    | qwen3 fsdp4    |           8607.3 |           8258.8 | **−4.05%** |        +4.37% → −0.82% |              97% |
    | llama2 fsdp4   |           4201.0 |           4118.7 |     −1.96% |        +4.80% → −1.17% |              43% |
    | llama2 emb2048 |          22612.1 |          22271.2 |     −1.51% |        +0.86% → −1.11% |             176% |
    | llama2 emb4096 |          73345.5 |          72611.7 |     −1.00% |        +1.61% → +0.25% |              63% |
    | mixtral fsdp4  |          10405.3 |          10327.2 |     −0.75% |        +3.05% → +0.35% |              25% |
    | llama2 tp4     |           4546.6 |           4573.1 |     +0.58% |        −0.31% → +0.13% | n/a (small loss) |

    Vocab-shaped entry copy bytes fall by exactly the predicted amount (dp4 441.5 → 48.3 MB,
    fsdp4 113.6 → 15.3, qwen3 482.0 → 15.3, emb4096 1474.4 → 687.9), and an op-class diff attributes
    ~100% of the speedup to data-movement (emb2048: −341.7 µs of a −340.9 µs total).

    The table above was produced with a harness monkeypatch. Re-running two of its configs through the
    shipped `lm_head_kernel_transposed` flag (2 reps each, same session) reproduces it:
    dp4 4430.2 / 4844.0 / 4420.1 µs (auto / explicit `[E,V]` / explicit `[V,E]`) = +9.34% → −0.23%,
    emb4096 72200.8 / 73330.0 / 72601.8 µs = +1.56% → +0.56%. The count of
    `f32[emb/n, 32000]` relayout copies in `after_optimizations` goes 6 → **0** on both, matching auto.

    **It defaults off, and must, for one reason:** the flag also changes the on-disk shape of
    `params-decoder-logits_dense-kernel`, so it equally hands `[V, embed]` to `auto` — where it is a
    *regression* (+3.96% llama2-fsdp4, +1.91% mixtral, +0.98% qwen3, +0.46%/+0.35% at emb 2048/4096,
    the small-model part being mechanism A reappearing on the auto side). Since `auto` is the default
    and the faster mode in 6 of 7 configs, `min(auto, explicit)` gets *worse* in 5 of 7 if the
    orientation is made global. **The follow-up that makes this default-on for explicit** is a
    restore-time transform that keeps the checkpoint at `[embed, vocab]` and transposes on load only
    when `shard_mode: explicit`; that keeps `auto` untouched and still delivers dp4 +9.35% → −0.22%,
    qwen3 +4.37% → +0.15%, emb2048 +0.86% → −0.65%, emb4096 +1.61% → +0.59%, fsdp4 +4.80% → +2.74%,
    mixtral +3.05% → +2.27%. It also needs `integration/tunix/utils.py:32` and the vLLM converter rule
    updated, since both hardcode the `[embed, vocab]` orientation and the Tunix one would *silently*
    mis-shard rather than raise.

    **Acceptance test:** zero `copy | f32[*,vocab]` instructions in the explicit
    `after_optimizations` dump. Note that `reduce-scatter` counts are **not** a good acceptance test
    — see the causal control in §4.2, which values that mechanism at ≤28% of the penalty.

02. **Add `shard_mode` to the golden suites (§7.3).** Until CI compares modes, every regression here
...
03. **Any HLO byte-counter used to compare modes must descend into
...
04. **Re-examine the `check_vma` ban (§7.1).** JAX 0.11.1 accepts `check_vma=True` on an all-Explicit
...
05. **Validate `fused_qkv` + explicit and context-parallel + explicit** in `types.py` so users get a
    MaxText message instead of a raw JAX trace error.

06. **Fix `gcs_utils.upload_dump()`** to no-op (or write locally) for non-`gs://` paths — today it
    crashes after training and destroys the profile.

07. **Make explicit mode not defeat remat CSE on unrolled blocks (§4.7).** With `remat_policy: full`
...
08. **Deduplicate the tied-embedding bf16 cast (§4.6).** `embeddings.py:235-240`'s `.T` makes explicit
...
09. **Investigate the tuple-repack all-reduce tax (§6.2)** — the one measured explicit cost that grows
    with depth (+112 → +219 → +379 µs from 4 to 12 layers on gemma3).

10. **Delete or rewire `validate_shard_mode`** (§7.4) — as dead code it actively misinforms.
